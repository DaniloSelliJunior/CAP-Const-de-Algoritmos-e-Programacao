/******************************************************************************
Descrição: Arquivos binarios e manipulação de registros
Requer:  
Assegura: 
...
void consulte_por_nome() {
    ...
    char entrada_nome[160];
    fgets(entrada_nome, sizeof entrada_nome, stdin);
    
    
    struct info pessoa;
    bool achou = false;
    while (!achou && fread(&pessoa, sizeof pessoa, 1, arquivo) > 0)
        if(strcmp(entrada_nome, pessoa.nome) == 0)
            achou = true;
            
    if(achou)
        printf("%s%s%.2f\n", pessoa.nome, pessoa.telefone, pessoa.salario);
    else
        printf("O nome registrado não foi encontrado\n");
